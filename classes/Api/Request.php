...
class S3ibusiness_Speedbox_Model_Api_Request
{
    ...
    public function __construct($args)
    {
        ...
        if ($is_auth) {
            // auth
            $auth                = new S3ibusiness_Speedbox_Model_Api_Authentication($args);
    ...
    public function dispatch()
    {
        ...
        if (null === $parsed_response) {
            ...
            throw new S3ibusiness_Speedbox_Model_Api_Exceptions(sprintf('Invalid JSON returned for %s.', $this->request->url), $this->response->code, $this->request, $this->response);
        }

        // any non-200/201/202 response code indicates an error
        if (!in_array($this->response->code, array('200', '201', '202'))) {

            throw new S3ibusiness_Speedbox_Model_Api_Exceptions(sprintf('Error: %s [%s]', $error_message, $error_code), $this->response->code, $this->request, $this->response);
        }
